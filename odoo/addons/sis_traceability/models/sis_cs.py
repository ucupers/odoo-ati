...
class sis_cs_header(models.Model):
    ...
    invoice = fields.Selection([('1','Fresh Fish'),('2','Frozen Fish'),('3','Frozen Loin/Flake')], default='1', string='Kategori', track_visibility="onchange")
    fresh_fish = fields.Boolean(string="Fresh", track_visibility="onchange")
    invoice_no = fields.Char(string="No. Invoice", size=255, track_visibility="onchange")
...
class sis_cs_detail(models.Model):
    ...
    fresh_fish = fields.Selection([('1','Fresh Fish'),('2','Frozen Fish'),('3','Frozen Loin/Flake')], string='Kategori', default='1')
    ...
    barcode_no = fields.Char(size=255,string="Barcode No", required=True)
    ...
    no_urut_loin = fields.Char(string="No Urut", compute="_list_no_urut_loin")
    ...
    @api.one
    @api.depends('cs_line_id.no_potong')
    def _get_nopotong(self):
        if self.cs_line_id.no_potong:
    ...
    def _constrains_barcode(self):
        xpabrik_id,xsection_id=self.cs_line_id._get_save_pabrik_id()
        if xsection_id=="Admin" or xsection_id=="Prod1" or xsection_id=="CSD" or xsection_id=="CS":
            print(xpabrik_id, xsection_id)
            qtyuse=0
            print(str(self.id))
...
#                 if (qtysisa1-qtyuse)<0 or self.quantity==0:
                if (qtysisa1-qtyuse)<self.quantity or self.quantity==0:
                    raise UserError("Qty. barcode "+self.barcode_no+" tidak mecukupi. Sisa Qtyxx : "+str(qtysisa1-qtyuse))
    #                raise UserError("Qty. barcode "+self.barcode_no+" tidak mecukupi. Sisa Qty : "+str(qtysisa1)+" "+str(qtyuse)+" "+str(self.quantity))
    ...
    def update_history(self):
        if self.barcode_no:
            if self.barcode_no[:3]=="AT/" or self.barcode_no[:3]=="AT2":
                xpabrik_id=self._get_save_pabrik_id()
                ...
                self.env.cr.execute(cSQL+"where barcode_no='"+self.barcode_no+"' and bg='"+xpabrik_id+"'")
                ...
                xpabrik_id=self._get_save_pabrik_id()
                cSQL="select barcode_no, fish_box_no,lot_no,item_no,description,vessel_no,hatch_no,voyage_no,quantity,remaining_quantity,status,status_analisa,status_process,histamin,kadar_garam,orange_meat,blackspot,pemakaian,kontaminasi,lain2_analisa,lain2_process,remark_analisa,remark_process,real_item_no,product_group_code,po,real_item_no from sis_fish_status "
                self.env.cr.execute(cSQL+"where fish_box_no='"+self.barcode_no+"' and bg='"+xpabrik_id+"'")
                rec_fish=self.env.cr.fetchall()
                if len(rec_fish)==1:
                    for cs_data in rec_fish:
                        (xbarcode_no, xfish_box,xno_lot,xitem_no,xdescription,xvessel_no,xhatch_no,xvoyage_no,xquantity,xremaining,xstatus,xstatus_analisa,xstatus_process,xhistamin,xkadargaram,xorangemeat,xblackspot,xpemakaian,xkontaminasi,xlainanalisa,xlainprocess,xremanalisa,xremprocess,xrealitem,xproductgroup,xpo,xrealitemno)=cs_data
                    
                    self.barcode_no=xbarcode_no
                    self.tgl_produksi = self.cs_line_id.tgl_produksi    
                    self.pabrik_id = self.cs_line_id.pabrik_id
                    if self.cs_line_id.invoice=='1':
                        if self.cs_line_id.fresh_fish==True:
                    ...
                    cSQLl="select barcode_no, fish_box_no,lot_no,item_no,remaining_quantity from sis_fish_status "
                    self.env.cr.execute(cSQLl+"where fish_box_no='"+self.barcode_no+"' and bg='"+xpabrik_id+"'")
                    rec_fish2=self.env.cr.fetchall()
                    raise UserError('Data ditemukan lebih dari satu : '+str(rec_fish2))
    ...
    def _get_data_fresh(self,kode):
        print(kode)
        if kode:
            cField="a.real_item_no,a.product_group_code,a.real_item_no"
            cSQL1="select "+cField+" from sis_fish_fresh_inv a left join sis_cs_detail b on a.invoice_no=b.barcode_no and a.quantity=b.quantity "
            cSQL2=" where b.barcode_no is null and a.invoice_no='"+str(self.cs_line_id.invoice_no)+"'"
            #print(cSQL1+cSQL2)
            self.env.cr.execute(cSQL1+cSQL2)
            fresh_fish_data=self.env.cr.fetchall()
            yrealitem=''
    ...
    @api.returns('self', lambda value:value.id)
    def create(self,vals):
#         cr_data=self.env['sis.cs.detail'].search([('barcode_no','=',vals['barcode_no']),('tgl_produksi','=',vals['tgl_produksi']),('no_potong','=',vals['no_potong']),('status_def','=',0)])
#         if len(cr_data)==0 or vals['fresh_fish']=='3':
            if vals['fresh_fish']!='1':
                ...
                xrealitem,xproductgroup,xrealitemno=self._get_data_fresh(vals['barcode_no'])                
                vals_cs={
                   'pemakaian':"",
                   'kontaminasi':"",
                   'lain2_analisa':"",
