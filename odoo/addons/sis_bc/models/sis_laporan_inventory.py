...
#                             xWhere=xWhere+" and ile."+xLocation_Code
    ...
    def get_data_bc(self):
        ...
        rbc=self.env['sis.report.filter.bc'].search([('rpt_name','=',self.rpt_laporan),('factory_code','!=',xFactory),('set_active','=',True)])
        if len(rbc)>0 :
#            xbg=""
            xwip=""
            buku_kuning=False
...
#                     (case when dnc.quantity is null then 0 else dnc.quantity end) as decrease,round( 
#                     (case 
