            ...
            for d in dets:
                ...
                if was!=None and d.description!=was.description:
                    if was.variant!=False and len(was.variant)>0:
                        ...
                        if len(its)==0:
                            raise UserError('ERROR: Item on SO does not exist on NAV Item Master')            
                    else:
...
#             sh=self.env['sis.shipment.header'].search([('no','=',no)])
    ...
    def get_item_so(self):
        iteminso={}
        self.env.cr.execute(" select distinct itemno, variant,quantity,qtyperuom,qtyshipped "+\
                            " from sis_so_line where docno='"+self.sono+"'")
    ...
    def release(self):
        if self.status=='draft':
            ...
            for h in hs:
                ...
                for d in h.detail_id:
                    num=self.env['sis.shipment.detail'].search([('status','!=','canceled'),('plno','!=',d.plno),('planlotno','=',d.planlotno)])
                    if len(num)>0:
                        for n in num:
    ...
    def check(self,check=True):
        ...
        for d in self.detail_id:
            self.env.cr.execute("select lot_no,quantity"+\
                        " from sis_temp_ile_rawfg where remaining_quantity>0 and lot_no='"+d.newlotno+"' and id!="+str(d.entryno))
            item=self.env.cr.fetchall()
            if len(item)>0:
                for itm in item:
                    (lot,quantity)=itm
...
#            no=self.env['ir.sequence'].next_by_code('sis.shipment.ati1.seq')
    ...
    def write(self, vals):
        if self.status =='stuffed':
            for v in vals:
                if v not in ['status','uploadremark']:
    ...
    def update_status_from_NAV(self):
        ...
        self.env.cr.execute(" select distinct plno, shipmentno, error "+\
                    " from sis_shipment_checking where shipmentno<>'' or error<>''")
        scs=self.env.cr.fetchall()
        for sc in scs:
            (plno,shipmentno,error)=sc
    ...
    def _onchange_qtyact(self):
        if self.plno and self.plno!='' :
            ...
            if self.lotno and self.lotno!='':
                num=self.env['sis.shipment.detail'].search([('status','!=','canceled'),('actlotno','=',self.lotno)])
                if len(num)>0:
                    for n in num:
