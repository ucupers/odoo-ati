...
class sis_temp_ile_remaining_quantity(models.Model):
    ...
    def _compute_plno(self):
        recs=self.env['sis.shipment.detail'].search(['|',('planlotno','=',self.lot_no),('newlotno','=',self.lot_no)],order='create_date desc',limit=1)
        for rec in recs:
            self.plno=rec.plno
...
#         self.env.cr.execute('update sis_goods_bin set out=sis_temp_ile_remaining_quantity set bin=gb.bin from sis_goods_bin gb where ile.id=gb.entry_no and gb.out=False')        
                
#         rs=self.env['sis.temp.ile.remaining.quantity'].search([])
#         for r in rs:
#             r.compute_bin()
...
#                     raise UserError('Lot not found!')
    ...
    def _compute_description(self):
        if self.ile!=None and self.ile!=False:
            ...
            ile=self.env['sis.temp.ile.remaining.quantity'].search([('lot_no','=',self.ile)])
    ...
    def create(self, vals):
        ile=self.env['sis.temp.ile.remaining.quantity'].search([('lot_no','=',vals['ile'])])
        if len(ile)==0:
...
class sis_goods_bin_free(models.Model):
    ...
    def upload_to_goods_bin(self):
        ...
        for r in rs:
            error=False
            ile=self.env['sis.temp.ile.remaining.quantity'].search([('lot_no','=',r['ile'])])
            if len(ile)==0:
    ...
    def open_report(self):
        ...
        for bin1 in bins:
            if bin1.find('*')==-1:
                ...
                wildbins+="or bin like '"+bin1.replace('*','%')+"' "
        ...
        sql="select bin,item_no,coalesce(variant, ''),description,sum(quantity),count(id)"+\
            "from sis_goods_bin where out=False and ( "
        if len(fixbins)>0:
            sql+=" bin in ("+fixbins+") "
        if len(wildbins)>0:
            if len(fixbins)>0:
        ...
        sql+=" ) group by bin,item_no,coalesce(variant, ''),description"
...
#         }
    ...
    def calc_qtyincase(self,itemno,variant,quantity):
        if itemno==False or len(itemno)==0:
            return
