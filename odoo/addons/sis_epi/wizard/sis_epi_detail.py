...
class sis_epi_detail(models.Model):
    ...
    # Cek qty
    @api.constrains('qty_fish_total')
    def cek_qty_fish(self):
        for rec in self:
            qty_fish = rec.qty_fish_total
            qty_target_fish = rec.yield_total_detail
            
            if qty_fish > qty_target_fish:
                raise UserError(_(
                             'ERROR!'))
...
class sis_epi_line_detail(models.Model):
    ...
    size_fish = fields.Selection([('sss', 'SSS'), ('ss', 'SS'), ('s', 'S'), ('m', 'M'), ('l', 'L'), 
                                  ('salmon', 'Salmon'), ('tonggol', 'Tonggol'), ('bonito', 'Bonito'), 
