...
#sql so 
            ...
            for it in range(1,32):
                if inv['a'+str(it)]==False:
                    if alt and len(alt)==1:
                        valsrun.update({'t'+str(it):valsrun['t'+str(it-1)]+prod['t'+str(it)]+alt['t'+str(it)]-sales['t'+str(it)]})
                    else:
    ...
    def calc_include_labeled(self,invori):
        ...
        labeled=self.env['sis.items.local'].search([('refitem','=',invori.item_no)])
...
#                s=self.env['sis.pps.detail'].search([('header_id','=',self.id),('detailnum','=',invori.detailnum),('type','=','sales')])
                ...
                a=self.env['sis.pps.detail'].search([('header_id','=',self.id),('detailnum','=',invori.detailnum),('type','=','alternative')])


                if self.ati12=='ati2':
                    sql= "select extract(day from postingdate) d, quantity, qtyperuom from sis_so_header_fg sh "+ \
                                " inner join sis_so_line_fg sl on sh.id=sl.header_id and extract(month from postingdate)="+str(self.month)+ \
                                " and extract(year from postingdate)="+str(self.year)+" and itemno='"+itemno+"' and bg='ATI2' " 
                    if variant:
                        sql+=" and variant='"+variant+"' "

                    sql+=" union select extract(day from itemrequireddate) d, quantity, qtyperuom from sis_pps_so_buffer sh "+ \
                                " where month ="+str(self.month)+ " and year="+str(self.year)+" and itemno='"+itemno+"' and upper(ati12)='ATI2' "
                    if variant:
                        sql+=" and variant='"+variant+"' "
                else:
                    sql= "select extract(day from postingdate) d, quantity, qtyperuom from sis_so_header_fg sh "+ \
                ...
                sql=" select d, sum(quantity), qtyperuom from ( "+sql+" ) a group by d, qtyperuom"
                ...
                for counter in range(1,32):
                    s.update({counter:0})
                for r in rs:
                    (sd,sqty,sqtyperuom)=r
                    s[sd]+=sqty*sqtyperuom/p.qtyperuom
                ...
                for counter in range (1,32):
                    if valsinv['a'+str(int(counter))]==True:
                        ...
                        if a and len(a)==1:
                            valsinv['t'+str(counter)]+=-s[counter]+a['t'+str(counter)]
                        else:                        
                            valsinv['t'+str(counter)]+=-s[counter]
...
#                     valsinv.update({'t'+str(counter):0})
    ...
    def countmaterial(self):
        ...
        self.env['sis.pps.detmaterial'].search([('header_id','=',self.id)]).unlink()        
        ...
        self.env.cr.execute('insert into sis_temp_production_bom select * from sis_production_bom')   
...
#        for sh in shs:
                    ...
                    for i in range(1,32):
                        ...
                        if d['t'+str(i)]!=0:
                            ...
                            if pl.variantuom==d.uom:
                                dettotal=pl.lineqtyper*d['t'+str(i)]
                            else:                                
                                dettotal=d['t'+str(i)]*d.qtyperuom/pl.variantqtyperuom*pl.lineqtyper
        ...
        for (litem,fish,item,desc,variant_code),v in detmaterial.items():
            s=self.env['sis.pps.material'].search([('item_no','=',litem),('fish','=',fish),('header','=',self.id)])
            dm=self.env['sis.pps.detmaterial'].search([('item_no','=',item),('variant_code','=',variant_code),('material_id','=',s.id)])
            v.update({'item_no':item,
            ...
            if len(dm)==1:
                ...
                if len(dm)==0:
                    self.env['sis.pps.detmaterial'].create(v)
                else:
                    raise UserError('Double material record !!')

        s=self.env['sis.pps.material'].search([('item_no','=','WIP-017'),('header','=',self.id)])
        if len(s)>0:                
            v1.update({'fish':'SHR',
                       'type':'TOTAL',
                       'uom':'KG',
    ...
    def calc_material_order_inventory(self):
        ...
        self.env.cr.execute("CREATE TEMP TABLE sis_ile_mat AS SELECT * FROM sis_ile where itc='PKG'; ")
                ...
                if capacity<total:
                    ...
                    for ppsitem in ppsitems:
                        ...
                        for order in orders:
                            ...
                            budomari=self.env['sis.budomari'].search([('month','=',self.month),('year','=',self.year),('ati12','=',self.ati12),('fish','=',fish1)]).budomari        
                            pb=self.env['sis.temp.production.bom'].search([('itemno','=',order.item_no),('variant','=',variant1),('lineitc','=','WIP'),('linepgc','!=','')])
...
#                 prod=self.env['sis.pps.detail'].search([('header_id','=',header_id),('detailnum','=',r.detailnum),('type','=','production')])
            ...
            for i in range(1,32):
                if inv['a'+str(i)]==False:
                    if r.type=='production':
                        if alt and len(alt)==1:
                            run+=(r['t'+str(i)]+alt['t'+str(i)]-s['t'+str(i)])
                            vals.update({'t'+str(i):run})
                        else:
                            run+=(r['t'+str(i)]-s['t'+str(i)])
                            vals.update({'t'+str(i):run})
                    else:
                        run+=(r['t'+str(i)]+prod['t'+str(i)]-s['t'+str(i)])
                        vals.update({'t'+str(i):run})
                else:
                    run=inv['t'+str(i)]
            inv.write(vals)
...
class sis_pps_material(models.Model):
    ...
    type = fields.Selection([('order','Order'),('material','Material'),('inventory','Inventory')],default='material',string="Type")

    fish = fields.Char(size=20,string="Fish Type")
    to = fields.Float(string="BegBal")
    ...
    def open_detmaterial(self): 
        return {
            'name': self.item_no +' - ' +self.description + ' - '+self.fish,
            'res_model': 'sis.pps.detmaterial',
            'type': 'ir.actions.act_window',
            'context': {},
            'view_mode': 'tree',
            'view_type': 'form',
...
#        self.header_id.countmaterial()


class sis_pps_detmaterial(models.Model):
    _name='sis.pps.detmaterial'
